...
describe("PreviewPage — DownloadCta deep-link URL contract (L16.1)", () => {
  for (const fmt of ["csv", "json", "xlsx", "md", "html", "txt", "jsonld", "rss", "atom"] as const) {
    it(`preferred format=${fmt} is the primary anchor, others follow`, async () => {
      ...
      // expect exactly 9 anchors (one per supported format), preferred first.
      expect(hrefs.length).toBe(9);
      expect(hrefs[0]).toContain(`format=${fmt}`);
      // the eight secondary anchors are the remaining formats; each appears
      // exactly once — pin the "filter doesn't double-emit / doesn't drop"
      // contract on the secondary slice.
      const rest = ["csv", "json", "xlsx", "md", "html", "txt", "jsonld", "rss", "atom"].filter(
        (f) => f !== fmt,
      );
...
describe("PreviewPage — format validation default (L16.1)", () => {
  ...
  it("format=html (valid, L38.3) → primary CTA passes through as html", async () => {
    // `html` is the L38.3 self-contained testimonials page; isFormat now
    // accepts it so the form's `format=html` rides through to the download
    // CTA verbatim, alongside md/json/csv/xlsx.
    const tree = await PreviewPage(
      mk({ placeId: "MOCK_SMALL_001", format: "html" }),
    );
    const primary = firstApiReviewsHref(tree);
    expect(primary).toContain("format=html");
  });

  it("format=txt (valid, L39.3) → primary CTA passes through as txt", async () => {
    // `txt` is the L39.3 unstyled plain-text testimonials document; isFormat
    // now accepts it so the form's `format=txt` rides through to the download
    // CTA verbatim, alongside md/html/json/csv/xlsx.
    const tree = await PreviewPage(
      mk({ placeId: "MOCK_SMALL_001", format: "txt" }),
    );
    const primary = firstApiReviewsHref(tree);
    expect(primary).toContain("format=txt");
  });

  it("format=jsonld (valid, L40.3) → primary CTA passes through as jsonld", async () => {
    // `jsonld` is the L40.3 schema.org structured-data export; isFormat now
    // accepts it so the form's `format=jsonld` rides through to the download
    // CTA verbatim, alongside md/html/txt/json/csv/xlsx.
    const tree = await PreviewPage(
      mk({ placeId: "MOCK_SMALL_001", format: "jsonld" }),
    );
    const primary = firstApiReviewsHref(tree);
    expect(primary).toContain("format=jsonld");
  });

  it("format=rss (valid, L41.3) → primary CTA passes through as rss", async () => {
    // `rss` is the L41.3 RSS 2.0 syndication feed; isFormat now accepts it so
    // the form's `format=rss` rides through to the download CTA verbatim,
    // alongside md/html/txt/json/csv/xlsx/jsonld.
    const tree = await PreviewPage(
      mk({ placeId: "MOCK_SMALL_001", format: "rss" }),
    );
    const primary = firstApiReviewsHref(tree);
    expect(primary).toContain("format=rss");
  });

  it("format=atom (valid, L42.3) → primary CTA passes through as atom", async () => {
    // `atom` is the L42.3 Atom 1.0 syndication feed; isFormat now accepts it so
    // the form's `format=atom` rides through to the download CTA verbatim,
    // alongside md/html/txt/json/csv/xlsx/jsonld/rss.
    const tree = await PreviewPage(
      mk({ placeId: "MOCK_SMALL_001", format: "atom" }),
    );
    const primary = firstApiReviewsHref(tree);
    expect(primary).toContain("format=atom");
  });
